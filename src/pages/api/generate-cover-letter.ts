...
import { generateCoverLetterSchema } from "@/lib/ai/types";
...
const handler = async (
  ...
  try {
    ...
    const dto = validateWithSchema(generateCoverLetterSchema, {
      resumeText,
      jobDescription: req.body.jobDescription,
    });

    const result = await aiService.generateCoverLetter(dto);

    return res.status(200).json(result);
  } catch (error) {
    if (error instanceof NotFoundError || error instanceof BadRequestError) {
      logEvent("error", "Error in generate-cover-letter handler", {
        error: error.message,
      });
